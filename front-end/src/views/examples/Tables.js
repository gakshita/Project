...
const Tables = () => {
  ...
  const getData = async () => {
    await axios
      .get(
        "https://raw.githubusercontent.com/gakshita/Mock-API/main/customerData"
      )
      .then(res => {
                  ...
                  {data.map(row => {
                    return (
                      <tr>
                        ...
                        <IconButton onClick={e => setOpenUserModal(row.id)}><PersonAddIcon></PersonAddIcon></IconButton>
                          <UncontrolledDropdown>
                            <DropdownToggle
